//
//  Heqz_MultiChannelContentView.m
//  HeComponent
//
...
@interface Heqz_MultiChannelContentView()<UIScrollViewDelegate>
...
@implementation Heqz_MultiChannelContentView
...
- (UIView*)loadContentViewWithIndex:(NSInteger)nIndex{
    ...
    Heqz_MultiChannelConfig* config = [self.arrayTabItem objectAtIndex:nIndex];
...
- (void)setArrayTabItem:(NSArray *)arrayTabItem{
    ...
    for(NSInteger i = 0; i < arrayTabItem.count; ++ i){
        Heqz_MultiChannelConfig* config = [arrayTabItem objectAtIndex:i];
        if(1 == config.status){
            selectedIndex = i;
            break ;
        }
