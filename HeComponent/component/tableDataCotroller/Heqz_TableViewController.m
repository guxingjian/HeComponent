...
@interface Heqz_TableViewController()
...
@implementation Heqz_TableViewController
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    if(self.arraySections.count > 0 && section < self.arraySections.count){
        Heqz_TableViewSectionConfig* sectionConfig = [self.arraySections objectAtIndex:section];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    if(self.arraySections.count > 0 && indexPath.section < self.arraySections.count){
        Heqz_TableViewSectionConfig* sectionConfig = [self.arraySections objectAtIndex:indexPath.section];
        if(indexPath.row < sectionConfig.arrayCells.count){
            Heqz_TableViewCellConfig* cellConfig = [sectionConfig.arrayCells objectAtIndex:indexPath.row];
            return cellConfig.cellHeight;
        }
    }else if(self.arrayCells.count > 0){
        if(indexPath.row < self.arrayCells.count){
            Heqz_TableViewCellConfig* cellConfig = [self.arrayCells objectAtIndex:indexPath.row];
            return cellConfig.cellHeight;
        }
    }
    return 0;
...
- (UITableViewCell*)cellWithCellConfig:(Heqz_TableViewCellConfig*)cellConfig tableView:(UITableView*)tableView{
    if(cellConfig.cell)
        return cellConfig.cell;
    
    UITableViewCell* cell = [tableView dequeueReusableCellWithIdentifier:cellConfig.cellIdentifier];
    if(!cell){
        cell = [Heqz_TableViewBaseCell tableViewCellWithNibName:cellConfig.cellName bundle:[NSBundle bundleForClass:[self class]]];
    }
    if(!cell){
    ...
    if([cell isKindOfClass:[Heqz_TableViewBaseCell class]]){
        Heqz_TableViewBaseCell* baseCell = (Heqz_TableViewBaseCell*)cell;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if([self.delegate respondsToSelector:@selector(tableController:tableCell:doActionWithInfo:)]){
        Heqz_TableViewBaseCell* cell = [tableView cellForRowAtIndexPath:indexPath];
        if([cell isKindOfClass:[Heqz_TableViewBaseCell class]]){
...
- (void)setTopLoadingView:(Heqz_LoadingView *)topLoadingView{
...
- (void)setBottomLoadingView:(Heqz_LoadingView *)bottomLoadingView{
...
- (void)processTopLoadingWithScrollView:(UIScrollView*)scrollView{
    ...
    if(Heqz_LoadingViewState_PrePareLoading == _topLoadingView.loadingState){
        [_topLoadingView startLoading];
    }else if(Heqz_LoadingViewState_Loading == _topLoadingView.loadingState){
        CGFloat fAdjust = 0;
        if(@available(iOS 11.0, *)){
    ...
    }else if(Heqz_LoadingViewState_Normal == _topLoadingView.loadingState){
        self.fEdgeTop = MAXFLOAT;
    }
}
...
- (void)processBottomLoadingWithScrollView:(UIScrollView*)scrollView{
    CGFloat fOffsetY = scrollView.contentOffset.y;
    UIEdgeInsets insets = self.tableView.contentInset;
    if(Heqz_LoadingViewState_PrePareLoading == _bottomLoadingView.loadingState){
        [_bottomLoadingView startLoading];
    }else if(Heqz_LoadingViewState_Loading == _bottomLoadingView.loadingState){
    ...
    }else if(Heqz_LoadingViewState_Normal == _bottomLoadingView.loadingState){
        self.fEdgeBottom = MAXFLOAT;
    }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if(_topLoadingView && !_topLoadingView.hidden && _topLoadingView.height > 0){
        ...
        if(fOffsetY < fPosY){
            if(Heqz_LoadingViewState_Normal == _topLoadingView.loadingState){
                if(fOffsetY < (fPosY - _topLoadingView.height) - Min_Equal_dis){
                    _topLoadingView.loadingState = Heqz_LoadingViewState_PrePareLoading;
                }
                _topLoadingView.loadingScale = (-_fEdgeTop - fAdjust - fOffsetY)/_topLoadingView.height;
            }else if(Heqz_LoadingViewState_PrePareLoading == _topLoadingView.loadingState){
                if(fOffsetY > (fPosY - _topLoadingView.height) + Min_Equal_dis){
                    _topLoadingView.loadingState = Heqz_LoadingViewState_Normal;
                }
                _topLoadingView.loadingScale = (-_fEdgeTop - fAdjust - fOffsetY)/_topLoadingView.height;
            }
        }
    ...
    if(_bottomLoadingView && !_bottomLoadingView.hidden && _bottomLoadingView.height > 0){
        ...
        if(fOffsetY > fTargetPosY){
            ...
            if(Heqz_LoadingViewState_Normal == _bottomLoadingView.loadingState){
                if(fOffsetY > fTargetPosY + _bottomLoadingView.height + Min_Equal_dis){
                    _bottomLoadingView.loadingState = Heqz_LoadingViewState_PrePareLoading;
                }
            }else if(Heqz_LoadingViewState_PrePareLoading == _bottomLoadingView.loadingState){
                if(fOffsetY < fTargetPosY + _bottomLoadingView.height - Min_Equal_dis){
                    _bottomLoadingView.loadingState = Heqz_LoadingViewState_Normal;
                }
            }
        }
    }
...
- (void)topLoadingViewChangeState:(Heqz_LoadingViewState)state{
    UIEdgeInsets insets = self.tableView.contentInset;
    if(Heqz_LoadingViewState_Loading == state){
    ...
    }else if(Heqz_LoadingViewState_EndLoading == state){
        weak_Self;
        [self setScrollViewContentInset:UIEdgeInsetsMake(self.fEdgeTop, insets.left, insets.bottom, insets.right) completeHanlder:^{
            weakSelf.topLoadingView.loadingState = Heqz_LoadingViewState_Normal;
            weakSelf.fEdgeTop = MAXFLOAT;
        }];
    }
}

- (void)bottomLoadingViewChangeState:(Heqz_LoadingViewState)state{
    UIEdgeInsets insets = self.tableView.contentInset;
    if(Heqz_LoadingViewState_Loading == state){
        ...
        self.bottomLoadingView.loadingState = Heqz_LoadingViewState_Normal;
        self.fEdgeBottom = MAXFLOAT;
...
- (void)scrollViewDidChangeAdjustedContentInset:(UIScrollView *)scrollView{
    if(Heqz_LoadingViewState_Loading != _bottomLoadingView.loadingState){
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context{
    if([keyPath isEqualToString:@"loadingState"]){
        Heqz_LoadingViewState state = [[change objectForKey:NSKeyValueChangeNewKey] integerValue];
